...
```{r scores_swbs}
swbs_scores_plot <- scores_plot("CW_SWBS", "CW-SWBS", "Mean CW-SWBS score (rescaled between 0 and 1)\n(lower = worse symptoms)", 0.6)
swbs_scores_plot
```
...
ggplot(plot_data, aes(x = intStartTime, y = CO2, colour = CO2)) +
